...
async function post(app: App, path: string, body: unknown, headers: Record<string, string> = {}): Promise<Response> {
...
describe('findMissingField', () => {
  let findMissingField: (body: Record<string, unknown>, fields: string[]) => string | null;
  ...
  it('returns null when all required fields are present', () => {
    expect(findMissingField({ email: 'a@b.com', password: 'secret' }, ['email', 'password'])).toBeNull();
  });

  it('returns missing field name when a field is missing', () => {
    expect(findMissingField({ email: 'a@b.com' }, ['email', 'password'])).toBe('password');
  });
...
describe('API routes', () => {
  ...
  describe('GET /api/auth/callback', () => {
    ...
    it('includes error_description in redirect when provided', async () => {
      const res = await app.request('/api/auth/callback?error=access_denied&error_description=User+cancelled');
      expect(res.status).toBe(302);
  ...
  describe('POST /api/auth/password — success', () => {
    it('returns authenticated with session cookie on valid credentials', async () => {
      ...
      expect(setCookies.some((c: string) => c.startsWith('wos-session='))).toBe(true);
    });
  });
  ...
  describe('GET /api/auth/session — with valid sealed session', () => {
    ...
    it('refreshes expired session and returns user data', async () => {
      ...
      const setCookies = res.headers.getSetCookie?.() ?? [];
      expect(setCookies.some((c: string) => c.includes('refreshed_sealed'))).toBe(true);
    });
  });
  ...
  describe('GET /api/auth/callback — success', () => {
    it('sets session cookie and redirects to frontend on valid code', async () => {
      ...
      const res = await app.request('/api/auth/callback?code=auth_code_123');
