...
const REQUIRED_ENV = ['WORKOS_API_KEY', 'WORKOS_CLIENT_ID', 'WORKOS_COOKIE_PASSWORD'] as const;
...
function csrfHash(token: string, sessionId: string): string {
  return createHmac('sha256', CSRF_SECRET).update(`${sessionId}!${token}`).digest('hex');
}

function generateCsrfToken(c: Parameters<MiddlewareHandler<AppEnv>>[0]): string {
...
function setSessionCookie(c: Parameters<MiddlewareHandler<AppEnv>>[0], sealedSession: string): void {
...
function handleOrgSelectionError(err: WorkOSError, c: Parameters<MiddlewareHandler<AppEnv>>[0]): Response | null {
...
function isSsoRequired(err: WorkOSError): boolean {
  const rawData = err.rawData || {};
  return rawData.error === 'sso_required' || err.error === 'sso_required' || err.code === 'sso_required';
}

function handleSsoRequiredError(err: WorkOSError, c: Parameters<MiddlewareHandler<AppEnv>>[0]): Response | null {
...
function findMissingField(body: Record<string, unknown>, fields: string[]): string | null {
...
const NO_REFRESH_REASONS = new Set(['no_session_cookie_provided', 'invalid_session_cookie']);
...
app.post('/api/auth/check-email', async (c) => {
  ...
  if (missing) return c.json({ status: 'error', error: `Missing required field: ${missing}` }, 400);
...
app.get('/api/auth/sso', async (c) => {
  ...
  if (!connectionId) {
    return c.redirect(`${FRONTEND_URL}?error=${encodeURIComponent('Missing connection ID')}`);
  }
...
app.post('/api/auth/org-selection', async (c) => {
  ...
  const missing = findMissingField(body, ['pendingAuthenticationToken', 'organizationId']);
  if (missing) return c.json({ status: 'error', error: `Missing required field: ${missing}` }, 400);

  try {
    const result = await workos.userManagement.authenticateWithOrganizationSelection({
